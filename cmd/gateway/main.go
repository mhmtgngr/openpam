...
func setupRouter(
	...
	v1 := r.Group("/api/v1")
	{
		...
		protected.Use(middleware2.Auth())
		{
			...
			protected.GET("/credentials/:id/reveal", middleware2.RequireRole("admin", "super_admin", "operator"), credentialHandler.Reveal)
			...
			protected.POST("/credentials/:id/rotate", middleware2.RequireRole("admin", "super_admin", "operator"), credentialHandler.Rotate)
			protected.POST("/credentials/:id/compromised", middleware2.RequireRole("admin", "super_admin", "operator"), credentialHandler.MarkCompromised)
			...
			protected.POST("/sessions/:id/terminate", middleware2.RequireRole("admin", "super_admin", "operator"), sessionHandler.Terminate)
			...
			protected.POST("/approvals/requests/:id/approve", middleware2.RequireRole("admin", "super_admin", "operator"), approvalHandler.Approve)
			protected.POST("/approvals/requests/:id/deny", middleware2.RequireRole("admin", "super_admin", "operator"), approvalHandler.Approve)
			protected.POST("/approvals/requests/:id/cancel", approvalHandler.Cancel)
			protected.POST("/approvals/requests/:id/delegate", middleware2.RequireRole("admin", "super_admin", "operator"), approvalHandler.Delegate)

			// Audit logs (require admin, super_admin, or auditor role)
			audit := protected.Group("/audit")
			audit.Use(middleware2.RequireRole("admin", "super_admin", "auditor"))
			{
				audit.GET("/events", auditHandler.List)
				audit.GET("/events/:id", auditHandler.Get)
				audit.GET("/export", auditHandler.Export)
				audit.GET("/integrity", auditHandler.VerifyIntegrity)
				audit.GET("/compliance/report", auditHandler.GenerateComplianceReport)
				audit.GET("/stats", auditHandler.GetStats)
			}

			// Discovery (require admin/super_admin)
			discovery := protected.Group("/discovery")
			discovery.Use(middleware2.RequireRole("admin", "super_admin"))
			{
				discovery.GET("/scans", handleListDiscoveryScans)
				discovery.POST("/scans", handleCreateDiscoveryScan)
				discovery.POST("/scans/:id/run", handleRunDiscoveryScan)
				discovery.GET("/assets", handleListDiscoveredAssets)
			}

			// Analytics (proxy to analytics-service, require admin/super_admin/auditor)
			analytics := protected.Group("/analytics")
			analytics.Use(middleware2.RequireRole("admin", "super_admin", "auditor"))
			{
				analytics.GET("/sessions", handleAnalyticsProxy)
				analytics.GET("/sessions/trends", handleAnalyticsProxy)
				analytics.GET("/sessions/summary", handleAnalyticsProxy)
				analytics.GET("/users/:id/activity", handleAnalyticsProxy)
				analytics.GET("/users/:id/risk", handleAnalyticsProxy)
				analytics.GET("/users/top", handleAnalyticsProxy)
				analytics.GET("/commands", handleAnalyticsProxy)
				analytics.GET("/commands/top", handleAnalyticsProxy)
				analytics.GET("/commands/blacklist", handleAnalyticsProxy)
				analytics.POST("/commands/blacklist", middleware2.RequireRole("admin", "super_admin"), handleAnalyticsProxy)
				analytics.PUT("/commands/blacklist/:id", middleware2.RequireRole("admin", "super_admin"), handleAnalyticsProxy)
				analytics.DELETE("/commands/blacklist/:id", middleware2.RequireRole("admin", "super_admin"), handleAnalyticsProxy)
				analytics.GET("/dashboard", handleAnalyticsProxy)
				analytics.GET("/timeseries", handleAnalyticsProxy)
				analytics.GET("/ssh-keys/:id", handleAnalyticsProxy)
			}

			// Compliance (proxy to analytics-service, require admin/super_admin/auditor)
			compliance := protected.Group("/compliance")
			compliance.Use(middleware2.RequireRole("admin", "super_admin", "auditor"))
			{
				compliance.GET("/reports", handleAnalyticsProxy)
				compliance.POST("/reports", middleware2.RequireRole("admin", "super_admin"), handleAnalyticsProxy)
				compliance.GET("/reports/:id", handleAnalyticsProxy)
				compliance.GET("/reports/:id/controls", handleAnalyticsProxy)
				compliance.GET("/summary", handleAnalyticsProxy)
				compliance.GET("/exceptions", handleAnalyticsProxy)
				compliance.POST("/exceptions", middleware2.RequireRole("admin", "super_admin"), handleAnalyticsProxy)
				compliance.PUT("/exceptions/:id/approve", middleware2.RequireRole("admin", "super_admin"), handleAnalyticsProxy)
				compliance.PUT("/exceptions/:id/deny", middleware2.RequireRole("admin", "super_admin"), handleAnalyticsProxy)
			}

			// Anomaly Detection (proxy to analytics-service, require admin/super_admin/auditor)
			anomalies := protected.Group("/anomalies")
			anomalies.Use(middleware2.RequireRole("admin", "super_admin", "auditor"))
			{
				anomalies.GET("", handleAnalyticsProxy)
				anomalies.GET("/:id", handleAnalyticsProxy)
				anomalies.POST("/detect", handleAnalyticsProxy)
				anomalies.PUT("/:id/status", middleware2.RequireRole("admin", "super_admin"), handleAnalyticsProxy)
				anomalies.GET("/trends", handleAnalyticsProxy)
				anomalies.GET("/users/:id", handleAnalyticsProxy)
			}

			// Ransomware Events (proxy to analytics-service, require admin/super_admin/auditor)
			ransomware := protected.Group("/ransomware")
			ransomware.Use(middleware2.RequireRole("admin", "super_admin", "auditor"))
			{
				ransomware.GET("/events", handleAnalyticsProxy)
				ransomware.GET("/events/:id", handleAnalyticsProxy)
				ransomware.POST("/events/:id/emergency", middleware2.RequireRole("admin", "super_admin"), handleAnalyticsProxy)
			}
