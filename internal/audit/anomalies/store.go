...
func (s *Store) Create(ctx context.Context, anomaly *model.AnomalyDetection) error {
	...
	// Generate correlation key if not set
	if anomaly.CorrelationKey == nil {
		correlationKey := s.generateCorrelationKey(anomaly)
		anomaly.CorrelationKey = &correlationKey
	}

	// Set correlation ID to match ID for first detection
	if anomaly.CorrelationID == nil {
		anomaly.CorrelationID = &anomaly.ID
	}
...
// CreateBatch creates multiple anomalies in a single transaction
func (s *Store) CreateBatch(ctx context.Context, anomalies []model.AnomalyDetection) error {
	...
	for i := range anomalies {
		...
		if anomalies[i].CorrelationKey == nil {
			correlationKey := s.generateCorrelationKey(&anomalies[i])
			anomalies[i].CorrelationKey = &correlationKey
		}

		if anomalies[i].CorrelationID == nil {
			anomalies[i].CorrelationID = &anomalies[i].ID
		}
...
func (s *Store) List(ctx context.Context, filter *AnomalyFilter, limit, offset int) ([]model.AnomalyDetection, int, error) {
...
func (s *Store) GetBySessionID(ctx context.Context, sessionID uuid.UUID) ([]model.AnomalyDetection, error) {
	var anomalies []model.AnomalyDetection
	query := `
		SELECT * FROM anomaly_detections
		WHERE session_id = $1
	...
	err := s.db.SelectContext(ctx, &anomalies, query, sessionID)
	if err != nil {
		return nil, fmt.Errorf("anomalies.GetBySessionID: %w", err)
	}
...
func (s *Store) GetByCorrelationID(ctx context.Context, correlationID uuid.UUID) ([]model.AnomalyDetection, error) {
	var anomalies []model.AnomalyDetection
	query := `
		SELECT * FROM anomaly_detections
	...
	err := s.db.SelectContext(ctx, &anomalies, query, correlationID)
	if err != nil {
		return nil, fmt.Errorf("anomalies.GetByCorrelationID: %w", err)
	}
...
func (s *Store) MergeDuplicateAnomalies(ctx context.Context, anomalyID uuid.UUID) (int, error) {
	// Get the target anomaly
	target, err := s.GetByID(ctx, anomalyID)
	if err != nil {
		return 0, err
	}

	if target.CorrelationKey == nil {
		return 0, fmt.Errorf("anomalies.MergeDuplicateAnomalies: target has no correlation key")
	}
	...
	query := `
		...
		SET is_duplicate = true,
			merged_into_id = $1,
			updated_at = NOW()
		WHERE correlation_key = $2
			AND id != $1
			AND is_duplicate = false
		RETURNING id
	`

	rows, err := s.db.QueryContext(ctx, query, anomalyID, *target.CorrelationKey)
	if err != nil {
		return 0, fmt.Errorf("anomalies.MergeDuplicateAnomalies: update: %w", err)
	}
	defer rows.Close()
	...
	updateCountQuery := `
		...
		WHERE id = $2
	`
	_, err = s.db.ExecContext(ctx, updateCountQuery, len(mergedIDs), anomalyID)
	if err != nil {
		return 0, fmt.Errorf("anomalies.MergeDuplicateAnomalies: update count: %w", err)
	}

	s.logger.Info().
		Str("target_id", anomalyID.String()).
		Int("merged_count", len(mergedIDs)).
		Msg("Anomalies merged")

	return len(mergedIDs), nil
}
...
func (s *Store) generateCorrelationKey(anomaly *model.AnomalyDetection) string {
	key := fmt.Sprintf("%s:%s", anomaly.AnomalyType, anomaly.TenantID.String())
	if anomaly.UserID != nil {
		key += ":" + anomaly.UserID.String()
	}
	if anomaly.TargetHost != nil {
		key += ":" + *anomaly.TargetHost
	}

	// Add hash of indicators for pattern matching
	if len(anomaly.Indicators) > 0 {
		var indicators map[string]interface{}
		if err := json.Unmarshal(anomaly.Indicators, &indicators); err == nil {
			if cmd, ok := indicators["command"].(string); ok && cmd != "" {
				key += ":" + cmd
			}
		}
	}
...
type AnomalyStats struct {
	...
	UniqueCorrelations int     `db:"unique_correlations" json:"unique_correlations"`
	TotalDuplicates    int     `db:"total_duplicates" json:"total_duplicates"`
